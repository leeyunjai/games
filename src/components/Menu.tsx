...
const DIFFS: { key: Difficulty; label: string; desc: string }[] = [
  ...
  { key: 'normal', label: '중간', desc: '2수 예측' },
  { key: 'hard', label: '어려움', desc: '4수 예측' },
];
    ...
    <div className="min-h-[100dvh] flex flex-col items-center justify-center px-4 py-8"
      ...
      <div className="w-full max-w-sm rounded-3xl p-6 space-y-5"
        ...
        <div className="flex items-center justify-between pt-1 border-t border-amber-900/40">
          <span className="text-amber-700 text-xs">효과음</span>
          <button onClick={() => setPref({ sound: !sound })} aria-pressed={sound}
            className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors focus-ring ${
              sound ? 'bg-amber-500/90 text-stone-900' : 'bg-stone-700 text-amber-300'
            }`}>
            {sound ? '켜짐' : '꺼짐'}
          </button>
        </div>
